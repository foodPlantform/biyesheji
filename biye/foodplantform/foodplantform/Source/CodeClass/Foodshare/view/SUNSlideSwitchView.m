...
static const CGFloat kWidthOfButtonMargin = 40.0f;
...
{
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0,self.bounds.size.width, 44)];
    _topScrollView.delegate = self;
    _topScrollView.backgroundColor = [UIColor clearColor];
    _topScrollView.pagingEnabled = NO;
    _topScrollView.showsHorizontalScrollIndicator = NO;
    _topScrollView.showsVerticalScrollIndicator = NO;
    _topScrollView.autoresizingMask =  UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleWidth;
...
- (void)createNameButtons
{
   ...
   // CGFloat xOffset = kWidthOfButtonMargin;
    ...
    for (int i = 0; i < [_viewArray count]; i++)
    {
        ...
        //HWUMainCategory *topCategoryModel = _dataArr[i];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        //获取button的动态宽度
        NSString *titleStr = _dataArr[i];
        CGSize textSize = [titleStr sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:(UIFont*)[UIFont systemFontOfSize:15],NSFontAttributeName, nil]];
