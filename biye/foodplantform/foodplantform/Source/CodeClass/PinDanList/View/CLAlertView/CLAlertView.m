...
- (void)setTitleArray:(NSArray *)titleArray{
    ...
    if (titleArray.count * 51 >= (ScreenHeight *2/3- 51)) {
        self.backView.frame = CGRectMake(0, ScreenHeight *1/3 - 51, ScreenWidth, ScreenHeight);
        
        self.tableView.frame = CGRectMake(0, ScreenHeight *1/3, ScreenWidth, ScreenHeight *2/3- 51);
        ...
        self.tableView.frame = CGRectMake(0, ScreenHeight -  self.titleArray.count * 51 - 51, ScreenWidth, titleArray.count * 51);
 ...
 *  上一级类目
 */
-(void)setLastRow:(NSInteger)lastRow
{
    _lastRow = lastRow;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_titleLabel.text isEqualToString:@"附近"])
    {
          cell.textLabel.text = [NSString stringWithFormat:@"%@km",self.titleArray[indexPath.row]];
    }else
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(globeBottomViewButtonClick:currentStr:lastRow:titleArr:)])
    {
        if ([_titleLabel.text isEqualToString:@"附近"])
        {
            [self.delegate globeBottomViewButtonClick:indexPath.row currentStr: [NSString stringWithFormat:@"%@km",self.titleArray[indexPath.row]]lastRow:_lastRow titleArr:self.titleArray];
        }else
        {
            [self.delegate globeBottomViewButtonClick:indexPath.row currentStr:_titleArray[indexPath.row]lastRow:_lastRow titleArr:self.titleArray];
        }
