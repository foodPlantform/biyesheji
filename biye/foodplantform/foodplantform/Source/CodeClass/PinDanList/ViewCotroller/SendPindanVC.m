...
//拼单食物名字
@property(nonatomic,strong)UILabel *foodPriceLB;
@property(nonatomic,strong)UITextField *foodPriceTf;
...
- (void)set_UpSendPindanView
{
    ...
    [self.view addSubview:_foodNameTf];
      ...
      _foodPriceLB= [[UILabel alloc] initWithFrame:CGRectMake(PinDanCellJianJu, CGRectGetMaxY(_foodNameLB.frame)+PinDanCellJianJu*2, CGRectGetWidth(_foodNameLB.frame), 10)];
    
    _foodPriceLB.text = [NSString stringWithFormat:@"大约价格: "] ;
    
    [self.view addSubview:_foodPriceLB];
    
    _foodPriceTf = [[UITextField alloc] initWithFrame:CGRectMake(CGRectGetMinX(_foodNameTf.frame), CGRectGetMinY(_foodPriceLB.frame)-10, kScreenWidth/3.0+20, 30)];
    _foodPriceTf.delegate = self;
    _foodPriceTf.keyboardType=UIKeyboardTypeNumberPad;

    _foodPriceTf.placeholder = @"美食的大约价格";
    [self.view addSubview:_foodPriceTf];
    _foodPersonLB = [[UILabel alloc] initWithFrame:CGRectMake(PinDanCellJianJu, CGRectGetMaxY(_foodPriceLB.frame)+PinDanCellJianJu*2, CGRectGetWidth(_foodNameLB.frame), 10)];
...
- (void)sendItemBtnAction
{
    if ([[FileManager shareManager] isUserLogin]) {
        ...
        if (0!=_foodPriceTf.text.length&&0!=_foodNameTf.text.length&&0!=_foodPersonLBTf.text.length&&0!=_foodPersonNumTf.text.length&&0!=_foodTimeTf.text.length&&0!=[_foodLocationBtn currentTitle].length)
        {
